...
In this report, we document the interesting results of our quanitative and qualitative analysis for various models behavior in Chart-to-Table task.
We show that Chart-to-Table task is a difficult task that needs either specialized model or very capable large general model to tackle this task.
Based on the selected sample of charts, we observed that the models can accurately recognize the layout of the graph, but they are not very precise in recognizing small differences in values.
Furthermore, there are types of charts that should be added to training data before asking models to extract data points from them like scatter plots.
We hope that our study fosters further research in this area.
